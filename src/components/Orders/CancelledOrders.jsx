...
const FetchcancelledOrders = () => {
  const { cancelledOrders, customers, refreshData, isLoading } =
    useContext(Context);
  ...
  return (
    <Box m="1.5rem 2.5rem">
      <Header
        ...
        subtitle={t("List of Cancelled Orders")}
      />
      <Typography>
        {t("Number of Cancelled Orders")}: {cancelledOrders.length}
      </Typography>
      <Box
        mt="40px"
        height="75vh"
      ...
      >
        <DataGrid
          ...
          loading={isLoading || !cancelledOrders}
          getRowId={(row) => row.id}
          columns={columns}
        />
      </Box>
...
export default FetchcancelledOrders;
